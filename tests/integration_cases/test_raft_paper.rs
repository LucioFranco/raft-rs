...
fn commit_noop_entry(r: &mut Interface, s: &MemStorage) {
    ...
    for m in msgs {
        assert_eq!(MessageType::from_i32(m.msg_type).unwrap(), MessageType::MsgAppend);
...
fn test_vote_request() {
    ...
    for (j, (ents, wterm)) in tests.drain(..).enumerate() {
        ...
        for (i, m) in msgs.iter().enumerate() {
            if MessageType::from_i32(m.msg_type).unwrap() != MessageType::MsgRequestVote {
                panic!(
                    "#{}.{}: msg_type = {:?}, want {:?}",
                    j,
                    i,
...
fn test_voter() {
    ...
    for (i, (ents, log_term, index, wreject)) in tests.drain(..).enumerate() {
        ...
        if msgs[0].msg_type != MessageType::MsgRequestVoteResponse as i32 {
            panic!(
                "#{}: msg_type = {:?}, want {:?}",
                i,
